...
st.markdown(
    ...
    <div style="line-height:1.1; padding-top:6px;">
      <div style="font-size:28px; font-weight:800;">대구고등법원</div>
...
# =========================================================
# Secrets 로드 (없으면 안내 후 종료)
# =========================================================
def require_secret(path_desc: str):
    st.error(
        f"Secrets 설정이 필요합니다: {path_desc}\n\n"
        "Streamlit Cloud → Manage app → Settings → Secrets 에 아래 형태로 추가하세요.\n\n"
        "[gsheets]\nspreadsheet_name=\"환경미화_소모품_장부\"\n\n"
        "[gcp_service_account]\n(type, client_email, private_key 등)\n"
    )
    st.stop()

if "gsheets" not in st.secrets or "spreadsheet_name" not in st.secrets["gsheets"]:
    require_secret('[gsheets] spreadsheet_name')

if "gcp_service_account" not in st.secrets:
    require_secret('[gcp_service_account] (서비스계정 JSON 내용)')
...
def gs_client():
    ...
    try:
        ...
        st.error(
            "Google 서비스계정 키(private_key) 형식 오류로 인증에 실패했습니다.\n\n"
            "대부분 Secrets의 private_key가 줄바꿈(엔터)로 깨졌을 때 발생합니다.\n"
            "private_key는 반드시 아래처럼 '\\n'이 포함된 '한 줄 문자열' 형태여야 합니다:\n\n"
            "private_key=\"-----BEGIN PRIVATE KEY-----\\n...\\n-----END PRIVATE KEY-----\\n\"\n\n"
            "Streamlit Cloud → Settings → Secrets에서 private_key를 JSON 원문 그대로 다시 붙여넣어 주세요."
        )
        st.stop()
...
def gs_open():
    ...
    try:
        ...
        st.error(
            "스프레드시트를 찾거나 열 수 없습니다.\n\n"
            "1) spreadsheet_name이 정확한지 확인\n"
            "2) 스프레드시트를 서비스계정 이메일(…iam.gserviceaccount.com)에게 '편집자'로 공유했는지 확인"
        )
        st.stop()
